...
export default class FormName extends React.Component {
  handleChangeText(e) {
    ...
    for (var i=0; i < title.length; i++) {
      ...
      if (plainCharacter >= 97 && plainCharacter <= 122) {
          text += String.fromCharCode((plainCharacter - 97 + number + 26) % 26 + 97);
      } else if (plainCharacter >= 65 && plainCharacter <= 90) {
          text += String.fromCharCode((plainCharacter - 65 + number + 26) % 26 + 65);
      } else {
  ...
  handleChangeVal(v) {
    ...
    for (var i=0; i < title.length; i++) {
      ...
      if (plainCharacter >= 97 && plainCharacter <= 122) {
          text += String.fromCharCode((plainCharacter - 97 + val + 26) % 26 + 97);
      } else if (plainCharacter >= 65 && plainCharacter <= 90) {
          text += String.fromCharCode((plainCharacter - 65 + val + 26) % 26 + 65);
      } else {
          text += String.fromCharCode(plainCharacter);
      }
    }
